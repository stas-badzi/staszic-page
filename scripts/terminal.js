...
xml.onreadystatechange = function() {
    if (xml.readyState == 4 && xml.status == 200) {
        if (gameid == -1) {
            ...
            setTimeout(function() {xml.open("GET", `/cgi-bin/update-game/${gameid}.cgi`); xml.send();}, 1000);
        } else {
            if (!gamelaunched) {
                let die = get_cookie(`die${gameid}`);
                gamelaunched = die && die[0] == '1';
            }
            FillScreen(decodeURIComponent(xml.responseText));
            ...
            keys = "";
            xml.open("GET", `/cgi-bin/update-game/${gameid}.cgi`);
            xml.send();
        }
    } else if (xml.readyState == 4 && (xml.status == 404 || xml.status == 500) && gameid != -1) {
        setTimeout(function() {xml.open("GET", `/cgi-bin/update-game/${gameid}.cgi`); xml.send();}, 250);
    }
};

set_cookie("args", stringifyJSONarray(argv));
xml.open("GET", `/cgi-bin/new-game.cgi`);
xml.send();
        ...
        if (c === '\x1b') {
            if (scr[i + 1] === '[') {
                ...
                if (full_action.turnon === 0) {
                    ...
                    if (full_action.actions[0].type === 6) string += "\x1b[" + (last_cursor.y + 1) + ';' + (last_cursor.x+1) + 'R';
...
function OnMousePress(e) {
    ...
    string += "\x1b[<" + operation + ";" + (mouse_column + 1) + ';' + (mouse_row+1) + 'M';
}
...
function OnMouseWheel(e) {
    ...
    if (deltaY != 0)
        string += "\x1b[<" + ((1<<6) | ((deltaY < 0) << 0)) + ";" + (mouse_column + 1) + ';' + (mouse_row+1) + "M";
    if (deltaX != 0) {
...
function UpdateToggledKeys(e) {
    ...
    if (new_toggled.CapsLock !== toggled_keys.CapsLock)
        keys += String.fromCharCode(new_toggled.CapsLock ? event_type.toggle_on : event_type.toggle_off, 0);
    if (new_toggled.NumLock !== toggled_keys.NumLock)
        keys += String.fromCharCode(new_toggled.CapsLock ? event_type.toggle_on : event_type.toggle_off, 1);
    if (new_toggled.ScrollLock !== toggled_keys.ScrollLock)
        keys += String.fromCharCode(new_toggled.CapsLock ? event_type.toggle_on : event_type.toggle_off, 2);
...
function OnKeyPress(e) {
    ...
    keys += String.fromCharCode(event_type.key_down, KeyChart[e.code]);
...
function OnKeyRelease(e) {
    ...
    //console.log("browser keycode: ", e.code);
    keys += String.fromCharCode(event_type.key_up, KeyChart[e.code]);
